...
<<<<<<< HEAD
        ...
        if (event.topic == 'chat_file') {
          ...
          if (type == 'file_chunk') {
            ...
            final total = decodedMap['total'] as int;
            final chunkData = decodedMap['data'] as String;
            final senderName = decodedMap['senderName'] as String? ?? '';

            if (!_incomingChatFileChunks.containsKey(fileId)) {
              _incomingChatFileChunks[fileId] = List<String?>.filled(total, null);
              _incomingChatFileNames[fileId] = fileName;
              _incomingChatSenderNames[fileId] = senderName;
            }
            _incomingChatFileChunks[fileId]![index] = chunkData;

            // Once all pieces arrive, decode and display in chat
            if (_incomingChatFileChunks[fileId]!.every((c) => c != null)) {
              final completeBase64 = _incomingChatFileChunks[fileId]!.join('');
              final fileBytes = base64Decode(completeBase64);
              final resolvedName = _incomingChatFileNames[fileId] ?? fileName;
              final resolvedSender = _incomingChatSenderNames[fileId] ?? '';

              // Cache bytes in memory so the preview dialog can access them
              _inMemoryChatFiles[fileId] = fileBytes;

              _incomingChatFileChunks.remove(fileId);
              _incomingChatFileNames.remove(fileId);
              _incomingChatSenderNames.remove(fileId);
              ...
              if (mounted) {
                setState(() {
                  _messages.add(
                    ChatMessage(
                      resolvedName,
                      false, // always from the remote side
                      ...
                      fileType: ChatMessage.detectType(resolvedName),
                    ),
                  );
                  if (!isChatOpen) {
                    _unreadMessageCount++;
        ...
        // 5. Standard chat text message processing
        if (event.topic == 'chat') {
          final text = decodedMap['text'] as String?;
