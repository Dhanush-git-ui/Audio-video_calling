...
class ScoringService {
  ...
  calculateLivenessScore(signals) {
    const reasons = [];
    let score = 1.0;
  ...
  calculateAntiSpoofScore(signals) {
    const reasons = [];
    let score = 1.0;
  ...
  async finalizeVerification(params) {
    ...
    const S_face = (sub.face !== undefined ? sub.face : 0.95) * 100;
    const S_iris = (sub.iris !== undefined ? sub.iris : 0.94) * 100;
    const S_liveness = (sub.liveness !== undefined ? sub.liveness : 0.96) * 100;
    const S_body = (sub.body !== undefined ? sub.body : 0.92) * 100;
    const S_antiSpoof = (sub.antiSpoof !== undefined ? sub.antiSpoof : 1.0) * 100;
